...
frontend/
  src/
    ...
    api/                     # generated client + types (see api-contract-sync)
...
## Data fetching
- Use the **typed client** generated by [[api-contract-sync]] (`src/api/`) — never hand-write
  request/response types that duplicate the backend.
- Default to a thin `fetch`-based client. **TanStack Query** is optional and recommended once
  caching/refetching needs appear; if added, keep query keys in `src/api/`.
- Read the API base URL from env, never hardcode.
